...
+ (NSArray *)lyricParserWithUrl:(NSString *)url isDelBlank:(BOOL)isDelBlank {
    // 根据歌词文件的url获取歌词内容
    if ([url hasPrefix:@"http"] || [url hasPrefix:@"https"]) { // 网络路径
        NSString *lyricStr = [NSString stringWithContentsOfURL:[NSURL URLWithString:url] encoding:NSUTF8StringEncoding error:nil];
        return [self lyricParaseWithLyricString:lyricStr isDelBlank:isDelBlank];
    }else { // 本地路径
        NSString *lyricStr = [NSString stringWithContentsOfFile:url encoding:NSUTF8StringEncoding error:nil];
        return [self lyricParaseWithLyricString:lyricStr isDelBlank:isDelBlank];
    }
}
...
/**
 ...
 @return 包含歌词模型的数组
 */
+ (NSArray *)lyricParserWithLocalPath:(NSString *)path isDelBlank:(BOOL)isDelBlank {
    // 根据歌词文件的url获取歌词内容
    NSString *lyricStr = [NSString stringWithContentsOfURL:[NSURL fileURLWithPath:path] encoding:NSUTF8StringEncoding error:nil];
    return [self lyricParaseWithLyricString:lyricStr isDelBlank:isDelBlank];
}
...
+ (NSArray *)lyricParaseWithLyricString:(NSString *)lyricString isDelBlank:(BOOL)isDelBlank {
    ...
    for (NSString *line in linesArray) {
        ...
        for (NSTextCheckingResult *match in matchesArray) {
            ...
            lyricModel.timeString   = [GKWYMusicTool timeStrWithMsTime:time];
